...
alldata = np.concatenate([allct, allcl, allmicro], axis = 1)
df = pd.DataFrame(data = alldata,
                  columns=['Trap Conc (ppm)', 'NILS conc (ppm)','Surface Area', 'Volume', 'Equivalent Diameter', 'Faces','Edges','Vertices'])

highcl = np.argwhere(allcl > 12)[:,0]
highcl_data = alldata[highcl]

grain = np.arange(0, 2600)



highclgrain = pd.DataFrame(data = highcl_data,
                           columns=['Trap Conc (ppm)', 'NILS conc (ppm)','Surface Area', 'Volume', 'Equivalent Diameter', 'Faces','Edges','Vertices'])

fig, axs = plt.subplots(3,2, sharex=True, figsize=(13,10))
axs[0, 0].scatter(grain, df['NILS conc (ppm)'].to_numpy(), s = 0.5, color = 'blue')
axs[0, 0].scatter(highcl, highclgrain['NILS conc (ppm)'].to_numpy(), s = 0.5, color = 'red')
axs[0, 0].set_title('NILS conc (ppm)')
axs[0, 1].scatter(grain, df['Surface Area'].to_numpy(), s = 0.5, color = 'blue')
axs[0, 1].scatter(highcl, highclgrain['Surface Area'].to_numpy(), s = 0.5, color = 'red')
axs[0, 1].set_title('Surface Area')
axs[1, 0].scatter(grain, df['Volume'].to_numpy(), s = 0.5, color = 'blue')
axs[1, 0].scatter(highcl, highclgrain['Volume'].to_numpy(), s = 0.5, color = 'red')
axs[1, 0].set_title('Volume')
axs[1, 1].scatter(grain, df['Equivalent Diameter'].to_numpy(), s = 0.5, color = 'blue')
axs[1, 1].scatter(highcl, highclgrain['Equivalent Diameter'].to_numpy(), s = 0.5, color = 'red')
axs[1, 1].set_title('Equivalent Diameter')
axs[2, 0].scatter(grain, df['Faces'].to_numpy(), s = 0.5, color = 'blue')
axs[2, 0].scatter(highcl, highclgrain['Faces'].to_numpy(), s = 0.5, color = 'red')
axs[2, 0].set_title('Faces')
